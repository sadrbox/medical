    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <form action="{{ url()->route('admin.article.update', ['article'=>$article->id]) }}" method="POST" enctype="multipart/form-data">
                    {{method_field('PUT')}}
                    {{csrf_field()}}
                                ...
                                <div class="form-group {{($errors->has('category_id')) ? 'has-error' : '' }}">
                                    ...
                                    <select name="category_id" id="category_id" class="form-control">
                                        ...
                                        @foreach($categories as $select)
                                            <option <?= ($article->category_id == $select->id) ? "selected" : "" ?> value="{{$select->id}}">{{$select->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
